Public Class GameController
    ...
    Private Sub SetActionsStates(context As IUIContext(Of IWorldModel))
        ...
        Next
        SetState(GameState.CookBagel, MakeActionState(context, Sub(m) m.Avatar.CookBagel()))
        SetState(GameState.BuildFire, MakeActionState(context, Sub(m) m.Avatar.BuildFire()))
        SetState(GameState.BuildFurnace, MakeActionState(context, Sub(m) m.Avatar.BuildFurnace()))
        SetState(GameState.Drop, MakeActionState(context, Sub(m) m.Item.Drop(), GameState.Inventory))
        SetState(GameState.Knap, MakeActionState(context, Sub(m) m.Avatar.Knap()))
        SetState(GameState.MakeHatchet, MakeActionState(context, Sub(m) m.Avatar.MakeHatchet()))
        SetState(GameState.MakeTorch, MakeActionState(context, Sub(m) m.Avatar.MakeTorch()))
        SetState(GameState.MakeTwine, MakeActionState(context, Sub(m) m.Avatar.MakeTwine()))
        SetState(GameState.PutOutFire, MakeActionState(context, Sub(m) m.Avatar.PutOutFire()))
